...
N_BEST, N_VAR, N_GEN, FREQ = 5, 10, 30, 1
STARTING_POP = 100
...
def main(checkpoint=None):
    ...
    for g in range(start_gen, N_GEN):
        ...
        if g % FREQ == 0:
            ...
            with open('data/evolution_long_run_02.pkl', 'wb') as cp_file:
                pickle.dump(cp, cp_file)

    gen = logbook.select('gen')
    logged_mins = logbook.select('min')
    logged_avgs = logbook.select('avg')

    fig, ax1 = plt.subplots()
    line1 = ax1.plot(gen, logged_mins, 'b-', label='Minimum Fitness')
    line2 = ax1.plot(gen, logged_avgs, 'r-', label='Average Fitness')
    ...
    plt.savefig('img/genetic_tests_02.png')


if __name__ == '__main__':
    main('evolution_long_run_02.pkl')
